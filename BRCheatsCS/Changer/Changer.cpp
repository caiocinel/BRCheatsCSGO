...
#include "../MemAlloc.h"
#include "Changer.h"
#include "Protobuffs/pbwrap.h"
#include "Protobuffs/protoparser.h"
#include "Protobuffs/protomessages.h"
...
namespace profile_changer {
	...
	bool send_client_hello() {
		...
		bool result = memory->SteamGameCoordinator->GCSendMessage(msg_client_hello | ((DWORD)1 << 31), ptr, packet.size() + 8) == EGCResult::k_EGCResultOK;
		free(ptr);
	...
	bool send_matchmaking_client_hello() {
		...
		memcpy((void*)((DWORD)ptr + 8), (void*)packet.data(), packet.size());
		bool result = memory->SteamGameCoordinator->GCSendMessage(msg_matchmaking_client_hello | ((DWORD)1 << 31), ptr, packet.size() + 8) == EGCResult::k_EGCResultOK;
		free(ptr);
	...
	bool send_client_rank_update() {
		...
		memcpy((void*)((DWORD)ptr + 8), (void*)packet.data(), packet.size());
		bool result = memory->SteamGameCoordinator->GCSendMessage(msg_client_rank_update | ((DWORD)1 << 31), ptr, packet.size() + 8) == EGCResult::k_EGCResultOK;
		free(ptr);
